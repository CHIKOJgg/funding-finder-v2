...
export async function scanAevo(): Promise<ExchangeResult[]> {
  try {
    ...
    const candidates = (Array.isArray(markets) ? markets : []).filter((m: any) => m?.instrument_type === 'PERPETUAL' || /-PERP$/.test(m?.instrument_name || '')).slice(0, 100);
    const results = await mapWithConcurrency(candidates, { concurrency: 3 }, async (m: any) => {
      try {
