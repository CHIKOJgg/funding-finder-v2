...
describe('calcFundingIncome (PnL simulation)', () => {
  it('long position accrues positive income when rate is positive', () => {
    ...
    expect(result.fundingIncome).toBeCloseTo(1000 * 0.0001 * 10, 5); // 1.0 USD
    expect(result.fundingIncome).toBeGreaterThan(0);
    expect(result.annualizedPct).toBeCloseTo(0.0001 * 24 * 365 * 100, 2); // ~8.76%
  });

  it('short position pays funding when rate is positive', () => {
    const result = calcFundingIncome({
      side: 'short',
      sizeUsd: 1000,
      leverage: 1,
